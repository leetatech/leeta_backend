...
type CartApplication interface {
	...
	ListCartItems(ctx context.Context, request domain.GetCartRequest) (*domain.ListCartResponse, error)
}
...
func (c CartAppHandler) ListCartItems(ctx context.Context, request domain.GetCartRequest) (*domain.ListCartResponse, error) {
	claims, err := c.tokenHandler.GetClaimsFromCtx(ctx)
	if err != nil {
		return nil, leetError.ErrorResponseBody(leetError.ErrorUnauthorized, err)
	}

	cart, err := c.allRepository.CartRepository.GetPaginatedCart(ctx, request, claims.UserID)
	if err != nil {
